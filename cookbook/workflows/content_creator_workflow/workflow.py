...
from ethosian.run.response import RunEvent
from ethosian.workflow import Workflow
...
class Tweet(BaseModel):
    ...
    media_urls: Optional[List[str]] = Field(
        default_factory=list, description="Associated media URLs, if any")  # type: ignore
...
class ContentPlanningWorkflow(Workflow):
    ...
    blog_analyzer: Agent = Agent(
        ...
        description=f"{agents_config['blog_analyzer']['role']
                       } - {agents_config['blog_analyzer']['goal']}",
        instructions=[
            f"{agents_config['blog_analyzer']['backstory']}",
            # Task-specific instructions for blog analysis
            tasks_config["analyze_blog"]["description"],
        ],
        # Expects response to follow the BlogAnalyzer Pydantic model
    ...
    twitter_thread_planner: Agent = Agent(
        model=OpenAIChat(id="gpt-4o"),
        description=f"{agents_config['twitter_thread_planner']['role']
                       } - {agents_config['twitter_thread_planner']['goal']}",
        instructions=[
    ...
    linkedin_post_planner: Agent = Agent(
        model=OpenAIChat(id="gpt-4o"),
        description=f"{agents_config['linkedin_post_planner']['role']
                       } - {agents_config['linkedin_post_planner']['goal']}",
        instructions=[
            f"{agents_config['linkedin_post_planner']['backstory']}",
            tasks_config["create_linkedin_post_plan"]["description"],
        ],
        # Expects response to follow the LinkedInPost Pydantic model
    ...
    def scrape_blog_post(self, blog_post_url: str, use_cache: bool = True):
        if use_cache and blog_post_url in self.session_state:
            ...
            if isinstance(response.content, BlogAnalyzer):
                ...
                raise ValueError(
                    "Unexpected content type received from blog analyzer.")
