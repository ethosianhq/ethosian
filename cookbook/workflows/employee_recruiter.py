...
from ethosian.run.response import RunResponse
from ethosian.tools.zoom import ZoomTool
...
from ethosian.agent.agent import Agent
from ethosian.model.openai.chat import OpenAIChat
from ethosian.tools.resend_tools import ResendTools
from ethosian.workflow.workflow import Workflow
...
class EmployeeRecruitmentWorkflow(Workflow):
    ...
    email_sender_agent: Agent = Agent(
        ...
        tools=[ResendTools(from_email="email@ethosian.com")],
    )
    ...
    def run(self, candidate_resume_urls: List[str], job_description: str) -> RunResponse:
        ...
        for resume_url in candidate_resume_urls:
            ...
            if resume_content:
                ...
                input = f"Candidate resume: {
                    resume_content}, Job description: {job_description}"
        ...
        for selected_candidate in selected_candidates:
            input = f"Schedule a 1hr call with Candidate name: {selected_candidate.name}, Candidate email: {
                selected_candidate.email} and the interviewer would be Manthan Gupts with email manthan@ethosian.com"
            ...
            if scheduled_call.content and scheduled_call.content.url and scheduled_call.content.call_time:
                input = f"Write an email to Candidate name: {selected_candidate.name}, Candidate email: {selected_candidate.email} for the call scheduled at {scheduled_call.content.call_time} with the url {
                    scheduled_call.content.url} and congratulate them for the interview from John Doe designation Senior Software Engineer and email john@ethosian.com"
                email = self.email_writer_agent.run(input)
                logger.info(email.content)

                if email.content:
                    input = f"Send email to {selected_candidate.email} with subject {
                        email.content.subject} and body {email.content.body}"
                    self.email_sender_agent.run(input)

        return RunResponse(
            content=f"Selected {len(selected_candidates)
