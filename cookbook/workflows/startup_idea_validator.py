"""
1. Install dependencies using: `pip install openai exa_py sqlalchemy ethosian`
...
from ethosian.tools.googlesearch import GoogleSearch
...
class MarketResearch(BaseModel):
    ...
    target_customer_segments: str = Field(...,
...
class StartupIdeaValidator(Workflow):
    ...
    def get_market_research(self, startup_idea: str, idea_clarification: IdeaClarification) -> Optional[MarketResearch]:
        agent_input = {"startup_idea": startup_idea,
    ...
    def run(self, startup_idea: str) -> Iterator[RunResponse]:
        logger.info(f"Generating a startup validation report for: {
                    startup_idea}")
        ...
        if idea_clarification is None:
            yield RunResponse(
                ...
                content=f"Sorry, could not even clarify the idea: {
                    startup_idea}",
            )
            return
...
if __name__ == "__main__":
    ...
    final_report: Iterator[RunResponse] = startup_idea_validator.run(
        startup_idea=idea)
