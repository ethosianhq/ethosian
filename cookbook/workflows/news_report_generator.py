"""
1. Install dependencies using: `pip install openai duckduckgo-search newspaper4k lxml_html_clean sqlalchemy ethosian`
...
from ethosian.workflow import Workflow, RunResponse, RunEvent
from ethosian.storage.workflow.sqlite import SqlWorkflowStorage
from ethosian.tools.duckduckgo import DuckDuckGo
from ethosian.tools.newspaper4k import Newspaper4k
from ethosian.utils.pprint import pprint_run_response
...
class NewsReportGenerator(Workflow):
    ...
    def get_search_results(self, topic: str, use_search_cache: bool) -> Optional[SearchResults]:
        ...
        if search_results is None:
            ...
            if (
                ...
                logger.info(f"WebSearcher identified {
                            len(web_searcher_response.content.articles)} articles.")
    ...
    def scrape_articles(self, search_results: SearchResults, use_scrape_cache: bool) -> Dict[str, ScrapedArticle]:
        ...
        if (
            ...
            for url, scraped_article in self.session_state["scraped_articles"].items():
                try:
                    ...
                    logger.warning(
                        f"Could not read scraped article from cache: {e}")
            logger.info(f"Found {len(scraped_articles)
                                 } scraped articles in cache.")
        ...
        for article in search_results.articles:
            ...
            if (
                ...
                logger.info(f"Scraped article: {
                            article_scraper_response.content.url}")
    ...
    def write_news_report(self, topic: str, scraped_articles: Dict[str, ScrapedArticle]) -> Iterator[RunResponse]:
        ...
        writer_input = {"topic": topic, "articles": [
            v.model_dump() for v in scraped_articles.values()]}
    ...
    def run(
        ...
        if search_results is None or len(search_results.articles) == 0:
            yield RunResponse(
                ...
                content=f"Sorry, could not find any articles on the topic: {
                    topic}",
            )
            return
