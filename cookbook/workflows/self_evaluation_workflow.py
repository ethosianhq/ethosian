...
class Feedback(BaseModel):
    content: str = Field(
        description="The content that you need to give feedback on")
    ...
    def run(self, topic: str) -> RunResponse:
        ...
        for _ in range(max_tries):
            ...
            if feedback.content and feedback.content.score > 8:
                break
            input = f"Here is the feedback: {feedback.content.feedback if feedback.content else ''} for your content {
                content.content if content.content else ''}. Please improve the content based on the feedback."
