...
from ethosian.tools.streamlit.components import get_username_sidebar
...
def main() -> None:
    ...
    # Get the LLM to use
    llm_id = st.sidebar.selectbox(
        "Select LLM", options=["gpt-4o", "gpt-4-turbo"])
    ...
    if personalized_assistant.storage:
        assistant_run_ids: List[str] = personalized_assistant.storage.get_all_run_ids(
            user_id=user_id)
        new_assistant_run_id = st.sidebar.selectbox(
            "Run ID", options=assistant_run_ids)
        if st.session_state["assistant_run_id"] != new_assistant_run_id:
            logger.info(
                f"---*--- Loading {llm_id} run: {new_assistant_run_id} ---*---")
