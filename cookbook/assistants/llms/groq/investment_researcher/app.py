...
st.markdown(
    "##### :orange_heart: Built using [ethosian](https://github.com/ethosianhq/ethosian)")
...
def main() -> None:
    # Get LLM Model
    model = (
        st.sidebar.selectbox("Select LLM", options=[
    ...
    get_analyst_recommendations = st.sidebar.checkbox(
        "Analyst Recommendations", value=True)
    get_upgrades_downgrades = st.sidebar.checkbox(
        "Upgrades/Downgrades", value=True)
    ...
    if generate_report:
            ...
            with st.status("Getting Company Info", expanded=True) as status:
                with st.container():
                    ...
                    if company_info_full:
                        ...
                        report_input += "---\n"
                status.update(label="Company Info available",
        ...
        if get_company_news:
            with st.status("Getting Company News", expanded=True) as status:
                with st.container():
                    ...
                    ddgs = DDGS()
                    company_news = ddgs.news(
                        keywords=ticker_to_research, max_results=5)
                    ...
                    if len(company_news) > 0:
                        ...
                        for news_item in company_news:
                            ...
                            if "date" in news_item:
                                company_news_md += f"  - Date: {
                                    news_item['date']}\n\n"
                            if "url" in news_item:
                                company_news_md += f"  - Link: {
                                    news_item['url']}\n\n"
                            if "source" in news_item:
                                company_news_md += f"  - Source: {
                                    news_item['source']}\n\n"
                        ...
                        report_input += "---\n"
                status.update(label="Company News available",
                              state="complete", expanded=False)
            ...
            with st.status("Getting Analyst Recommendations", expanded=True) as status:
                with st.container():
                    ...
                    if not analyst_recommendations.empty:
                        analyst_recommendations_container.write(
                    ...
                    report_input += "---\n"
                status.update(label="Analyst Recommendations available",
                              state="complete", expanded=False)
            ...
            with st.status("Getting Upgrades/Downgrades", expanded=True) as status:
                with st.container():
                    ...
                    if not upgrades_downgrades.empty:
                        upgrades_downgrades_container.write(
                            upgrades_downgrades)
                    ...
                    report_input += "---\n"
                status.update(label="Upgrades/Downgrades available",
                              state="complete", expanded=False)
        ...
        with st.status("Generating Draft", expanded=True) as status:
            ...
            status.update(label="Draft Generated",
                          state="complete", expanded=False)

        with st.spinner("Generating Report"):
            final_report = ""
            final_report_container = st.empty()
            report_message = f"Please generate a report about: {
                ticker_to_research}\n\n\n"
