...
from ethosian.document.reader.pdf import PDFReader
from ethosian.document.reader.website import WebsiteReader
...
def main() -> None:
    ...
    search_type = st.sidebar.selectbox("Select Search Type", options=[
                                       "Knowledge Base", "Web Search (Tavily)"])
    ...
    if "research_assistant" not in st.session_state or st.session_state["research_assistant"] is None:
        research_assistant = get_rag_research_assistant(
            model=model, num_documents=num_documents)
            ...
            if input_url is not None:
                ...
                if f"{input_url}_scraped" not in st.session_state:
                    scraper = WebsiteReader(
                        chunk_size=3000, max_links=5, max_depth=1)
    ...
    if generate_report:
        ...
        if search_type == "Knowledge Base" and research_assistant.knowledge_base:
            with st.status("Searching Knowledge", expanded=True) as status:
                with st.container():
                    ...
                    if len(kb_search_docs) > 0:
                        ...
                        for idx, doc in enumerate(kb_search_docs):
                            ...
                            kb_search_results += f"{
                                json.dumps(doc.meta_data, indent=4)}\n\n"
                ...
                status.update(label="Knowledge Search Complete",
            ...
            with st.status("Searching Web", expanded=True) as status:
                ...
                status.update(label="Web Search Complete",
                              state="complete", expanded=False)

        if not topic_search_results:
            st.write(
                "Sorry could not generate any search results. Please try again.")
            return

        with st.spinner("Generating Report"):
            final_report = ""
            final_report_container = st.empty()
            report_message = f"Task: Please generate a report about: {
                input_topic}\n\n"
            report_message += f"Here is more information about: {
                input_topic}\n\n"
