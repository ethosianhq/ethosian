...
from ethosian.run.response import RunEvent, RunResponse
from ethosian.tools.linear_tools import LinearTool
from ethosian.tools.slack import SlackTools
from ethosian.agent.agent import Agent
from ethosian.workflow.workflow import Workflow
from ethosian.storage.workflow.postgres import PgWorkflowStorage
...
class Task(BaseModel):
    ...
    task_assignee: Optional[str] = Field(
        None, description="The assignee of the task")
...
class LinearIssue(BaseModel):
    ...
    issue_assignee: Optional[str] = Field(
        None, description="The assignee of the issue")
    issue_link: Optional[str] = Field(
        None, description="The link to the issue")
...
class ProductManagerWorkflow(Workflow):
        ...
        while tasks is None and num_tries < 3:
            ...
            try:
                ...
                if response and response.content and isinstance(response.content, TaskList):
                    ...
                    logger.warning(
                        "Invalid response from task agent, trying again...")
    ...
    def create_linear_issues(self, tasks: TaskList, linear_users: Dict[str, str]) -> Optional[LinearIssueList]:
        ...
        linear_response: RunResponse = self.linear_agent.run(
            f"Create issues in Linear for project {project_id} and team {team_id}: {tasks.model_dump_json(
            )} and here is the dictionary of users and their uuid: {linear_users}. If you fail to create an issue, try again."
        )
        linear_issues = None
        if linear_response:
    ...
    def run(self, meeting_notes: str, linear_users: Dict[str, str], use_cache: bool = False) -> RunResponse:
        ...
        if linear_issues:
            logger.info(f"Sending slack notification with tasks: {
...
meeting_notes = open(
    "cookbook/examples/product_manager_agent/meeting_notes.txt", "r").read()
users_uuid = {
