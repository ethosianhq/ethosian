...
st.title("Paperpal")
st.markdown(
    "##### :orange_heart: built by [ethosian](https://github.com/ethosianhq/ethosian)")
        ...
        with st.status("Generating Search Terms", expanded=True) as status:
            with st.container():
                ...
                search_generator_input = {
                    "topic": report_topic, "num_terms": num_search_terms}
                search_terms = search_term_generator.run(
                    json.dumps(search_generator_input)).content
                if search_terms:
                    search_terms_container.json(search_terms.model_dump())
            status.update(label="Search Terms Generated",
                ...
                with st.container():
                    ...
                    try:
                        ...
                        if (
                            ...
                            exa_content = exa_search_results.model_dump_json(
                                indent=4)
                            exa_container.json(
                                exa_search_results.content.results)
                            status.update(label="Exa Search Complete",
                                          state="complete", expanded=False)
                    except Exception as e:
                        st.error(f"An error occurred during Exa search: {e}")
                        status.update(label="Exa Search Failed",
                ...
                with st.container():
                    ...
                    if arxiv_search_results and arxiv_search_results.content and arxiv_search_results.content.results:
                        arxiv_container.json(
                            [result.model_dump() for result in arxiv_search_results.content.results])
                status.update(label="ArXiv Search Complete",
                              state="complete", expanded=False)
            ...
            if arxiv_search_results and arxiv_search_results.content and arxiv_search_results.content.results:
                ...
                if paper_summaries:
                    with st.status("Displaying ArXiv Paper Summaries", expanded=True) as status:
                        ...
                        status.update(
                            label="ArXiv Paper Summaries Displayed", state="complete", expanded=False)

                    arxiv_paper_ids = [summary["ID"]
                    ...
                    if arxiv_paper_ids:
                        with st.status("Reading ArXiv Papers", expanded=True) as status:
                            with st.container():
                                ...
                                st.write(
                                    f"Read {len(arxiv_paper_ids)} ArXiv papers")
                            status.update(
                                label="Reading ArXiv Papers Complete", state="complete", expanded=False)
