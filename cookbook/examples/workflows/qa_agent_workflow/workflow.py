...
from ethosian.vectordb.search import SearchType
from ethosian.embedder.openai import OpenAIEmbedder
from ethosian.document.chunking.recursive import RecursiveChunking
...
class QAWorkflow(Workflow):
    ...
    def run(self, evaluation_path, output_path, knowledge_base_recreate=True):
        ...
        logger.info(f"Loading of the website done in {
                    duration_load:.2f} seconds")
...
if __name__ == "__main__":
    ...
    logger.info(f"The question generation and evaluation workflow is completed in {
                duration_flow:.2f} seconds")
