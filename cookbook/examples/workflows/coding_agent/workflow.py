...
from ethosian.workflow import Workflow, RunEvent
        ...
        while attempt < self.max_retries:
            ...
            try:
                if attempt == 0:
                    ...
                    response = self.coding_agent.run(
                ...
                logger.error(
                    f"---CODE BLOCK CHECK: FAILED IN ATTEMPT {attempt + 1}---")
                error_message = str(e)
...
if __name__ == "__main__":
    ...
    final_content = workflow.run(
        question=question, context=concatenated_content)
