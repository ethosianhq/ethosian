"""
1. Install dependencies using: `pip install ethosian opencv-python google-generativeai sqlalchemy pydantic`
...
def extract_segments(response_text):
    ...
    for match in re.finditer(segments_pattern, str(response_text)):
        ...
        start_seconds = sum(x * int(t)
                            for x, t in zip([60, 1], start_time.split(":")))
        end_seconds = sum(x * int(t)
                          for x, t in zip([60, 1], end_time.split(":")))
