...
def display_header(
    ...
    panel = Panel(Align.center(title), title=panel_title,
                  subtitle=subtitle, border_style=border_style, padding=(1, 2))
    console.print(panel)
...
def display_content(content, title: str = "Content"):
    ...
    panel = Panel(pretty_content, title=title,
...
def run_agents():
    try:
        ...
        display_header(
            "Running Agent with response_model=MovieScript", panel_title="Agent 1")
        ...
        with console.status("Running Agent 2...", spinner="dots"):
            run_structured_output_agent: RunResponse = structured_output_agent.run(
                "New York")
        display_content(run_structured_output_agent.content,
                        title="Agent 2 Response")
    except Exception as e:
        console.print(
            f"[bold red]Error occurred while running agents: {e}[/bold red]")
...
async def run_agents_async():
    try:
        ...
        display_header(
            "Running Agent with response_model=MovieScript (async)", panel_title="Async Agent 1")
        with console.status("Running Agent 1...", spinner="dots"):
            async_run_json_mode_agent: RunResponse = await json_mode_agent.arun("New York")
        display_content(async_run_json_mode_agent.content,
                        title="Async Agent 1 Response")
        ...
        display_content(async_run_structured_output_agent.content,
                        title="Async Agent 2 Response")
    except Exception as e:
        console.print(
            f"[bold red]Error occurred while running async agents: {e}[/bold red]")
