"""Run `pip install openai exa_py duckduckgo-search yfinance pypdf sqlalchemy 'fastapi[standard]' ethosian youtube-transcript-api` to install dependencies."""
...
web_agent = Agent(
    ...
    tools=[DuckDuckGo()],
    instructions=[
        "Break down the users request into 2-3 different searches.", "Always include sources"],
...
image_agent = Agent(
    ...
    model=OpenAIChat(id="gpt-4o"),
    tools=[Dalle(model="dall-e-3", size="1792x1024",
...
research_agent = Agent(
    ...
    model=OpenAIChat(id="gpt-4o"),
    tools=[ExaTools(start_published_date=datetime.now().strftime(
        "%Y-%m-%d"), type="keyword")],
    description=(
    ...
    - [Reference 2](link)
    """),
    storage=SqlAgentStorage(table_name="research_agent",
                            db_file=agent_storage_file),
    add_history_to_messages=True,
    add_datetime_to_instructions=True,
    markdown=True,
...
youtube_agent = Agent(
    ...
    add_datetime_to_instructions=True,
    storage=SqlAgentStorage(table_name="youtube_agent",
                            db_file=agent_storage_file),
    markdown=True,
)

app = Playground(agents=[web_agent, finance_agent,
                 youtube_agent, research_agent, image_agent]).get_app()
