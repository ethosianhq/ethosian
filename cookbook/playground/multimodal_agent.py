"""
1. Install dependencies: `pip install openai sqlalchemy 'fastapi[standard]' ethosian requests`
2. Authenticate with ethosian: `ethosian auth`
3. Run the agent: `python cookbook/playground/multimodal_agent.py`

Docs on Agent UI: https://docs.ethosianhq.com/agent-ui
"""

from ethosian.agent import Agent
from ethosian.model.openai import OpenAIChat
from ethosian.tools.dalle import Dalle
from ethosian.tools.eleven_labs_tools import ElevenLabsTools
from ethosian.tools.giphy import GiphyTools
from ethosian.tools.models_labs import ModelsLabs
from ethosian.model.response import FileType
from ethosian.playground import Playground, serve_playground_app
from ethosian.storage.agent.sqlite import SqlAgentStorage
from ethosian.tools.fal_tools import FalTools
from ethosian.tools.desi_vocal_tools import DesiVocalTools
...
image_agent = Agent(
    ...
    storage=SqlAgentStorage(table_name="image_agent",
...
ml_gif_agent = Agent(
    ...
    add_history_to_messages=True,
    add_datetime_to_instructions=True,
    storage=SqlAgentStorage(table_name="ml_gif_agent",
                            db_file=image_agent_storage_file),
)
...
ml_video_agent = Agent(
    ...
    add_history_to_messages=True,
    add_datetime_to_instructions=True,
    storage=SqlAgentStorage(table_name="ml_video_agent",
                            db_file=image_agent_storage_file),
)
...
fal_agent = Agent(
    ...
    add_history_to_messages=True,
    add_datetime_to_instructions=True,
    storage=SqlAgentStorage(table_name="fal_agent",
                            db_file=image_agent_storage_file),
)
...
gif_agent = Agent(
    ...
    add_history_to_messages=True,
    add_datetime_to_instructions=True,
    storage=SqlAgentStorage(table_name="gif_agent",
                            db_file=image_agent_storage_file),
)
...
audio_agent = Agent(
    ...
    add_history_to_messages=True,
    add_datetime_to_instructions=True,
    storage=SqlAgentStorage(table_name="audio_agent",
                            db_file=image_agent_storage_file),
)
...
image_to_image_agent = Agent(
    ...
    storage=SqlAgentStorage(table_name="image_to_image_agent",
                            db_file=image_agent_storage_file),
)
...
hindi_audio_agent = Agent(
    ...
    add_history_to_messages=True,
    add_datetime_to_instructions=True,
    storage=SqlAgentStorage(table_name="hindi_audio_agent",
                            db_file=image_agent_storage_file),
)
