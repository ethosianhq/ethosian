...
from ethosian.tools.discord_tools import DiscordTools
...
discord_agent.print_response(
    f"Send a message 'Hello from ethosian!' to channel {channel_id}", stream=True)

# Example 2: Get channel info
discord_agent.print_response(f"Get information about channel {
                             channel_id}", stream=True)

# Example 3: List channels
discord_agent.print_response(f"List all channels in server {
                             server_id}", stream=True)

# Example 4: Get message history
discord_agent.print_response(f"Get the last 5 messages from channel {
                             channel_id}", stream=True)
