...
from ethosian.memory.agent import AgentMemory
from ethosian.memory.db.mongodb import MongoMemoryDb
...
agent = Agent(
    ...
    memory=AgentMemory(
        db=MongoMemoryDb(collection_name="agent_sessions",
                         db_url=db_url, db_name="ethosian"),
...
def print_chat_history(agent):
    ...
    console.print(
        Panel(
            JSON(json.dumps([m.model_dump(
                include={"role", "content"}) for m in agent.memory.messages]), indent=4),
