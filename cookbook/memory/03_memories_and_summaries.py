...
Steps:
1. Run: `pip install openai sqlalchemy ethosian` to install dependencies
...
from ethosian.agent import Agent, AgentMemory
from ethosian.model.openai import OpenAIChat
from ethosian.memory.db.sqlite import SqliteMemoryDb
...
def print_agent_memory(agent):
    ...
    console.print(
        render_panel(
            ...
            json.dumps([m.model_dump(include={"role", "content"})
                       for m in agent.memory.messages], indent=4),
        )
    )
    # -*- Print memories
    console.print(
        render_panel(
            ...
            json.dumps([m.model_dump(include={"memory", "input"})
                       for m in agent.memory.memories], indent=4),
        )
    )
    # -*- Print summary
    console.print(
        render_panel(
            f"Summary for session_id: {agent.session_id}", json.dumps(
                agent.memory.summary.model_dump(), indent=4)
        )
    )
...
agent.print_response(
    "My name is john billings and I live in nyc.", stream=True)
# -*- Print agent memory
